...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(in_channels=c_in, out_channels=d_model, kernel_size=3, stride=1, padding=1),
            nn.ReLU(),
        ...
        self.model = PatchTST_backbone(
            c_in=d_model, context_window=context_window, target_window=target_window, patch_len=patch_len, stride=stride,
    ...
    def forward(self, x):  # x: [Batch, Input length, Channel]
        ...
        x = self.model(x)  # PatchTST backbone
